...
A slot's own owner claim, written by the spawn that takes it under the allocation lock and owned by [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh), is read before that record scan and settles reassignment whether or not the claimant left a record: a claim naming a different task releases nothing - teardown warns, names the claimant, and finishes only the task's own cleanup - and a claim naming this task marks an older crewmate record for the same slot as stale once that record's endpoint reads dead or missing (a live one still refuses) and records `slot_reassigned_to=` on it, which stands in for the claim once the slot is back in the pool so a later relaunch refuses and that record's own teardown releases nothing, so two surviving records cannot deadlock each other's teardown; the claim exists because Treehouse's own live process lease cannot answer ownership once the worker's exit releases it.
