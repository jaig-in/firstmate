...
`config/primary-harness` accepts only a verified harness adapter name (`claude codex opencode pi pi-signed grok kimi cursor omp`; the crew-only adapters `muse gemini rovo agy` are refused), and any other value - including a symlinked file or a value with interior whitespace - fails loudly at launch.
That whitelist keeps a home config from naming an arbitrary binary; it is a name check, not a support claim - `kimi` passes it while carrying no primary supervision protocol ([Harness support](#harness-support)), and [README requirements](../README.md#requirements) own the harnesses supported for a primary session.
