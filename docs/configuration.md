...
When it differs from the install root, `bin/fm-session-start.sh` emits a LAUNCH CONTEXT section naming the launch directory, the enclosing git repository as the working project, the registered alias, `unregistered`, or an unreadable registry, and the path of that repository's `AGENTS.md` or `CLAUDE.md`; a bounded, per-line-capped excerpt of that file follows the CONTEXT digest.
A launch outside any git repository claims no working project (`Working project: none`) and names an `AGENTS.md` or `CLAUDE.md` in the launch directory itself when one exists.
